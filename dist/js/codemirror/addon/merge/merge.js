  ...
  TrackAlignable.prototype = {
    ...
    set: function(n, flags) {
      ...
      for (; pos < this.alignable.length; pos += 2) {
        var diff = this$1.alignable[pos] - n
        if (diff == 0) {
          if ((this$1.alignable[pos + 1] & flags) == flags) { return }
          this$1.alignable[pos + 1] |= flags
          this$1.signal()
          return
        }
        if (diff > 0) { break }
    ...
    find: function(n) {
      ...
      for (var i = 0; i < this.alignable.length; i += 2)
        { if (this$1.alignable[i] == n) { return i } }
      return -1
    },
    ...
    map: function(from, nBefore, nAfter) {
      ...
      for (var i = 0; i < this.alignable.length; i += 2) {
        var n = this$1.alignable[i]
        if (n == from && (this$1.alignable[i + 1] & F_WIDGET_BELOW)) { widgetFrom = i }
        if (n == to && (this$1.alignable[i + 1] & F_WIDGET_BELOW)) { widgetTo = i }
        if (n <= from) { continue }
        else if (n < to) { this$1.alignable.splice(i--, 2) }
        else { this$1.alignable[i] += diff }
      }
      if (widgetFrom > -1) {
